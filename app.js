...
// Lastfaktoren für die fünf Zyklen
const ZYKLUS_DEF=[
...
function uid(){try{return crypto.randomUUID();}catch{return 'id_'+Date.now()+'_'+Math.random().toString(16).slice(2);}}
...
const timerMap={};
let _saveT=null,_audioCtx=null,_alarmGain=null,_alarmReady=false,_floatingRaf=null,_timeAdjustVid=null;
...
function defaultZyklus(nr){
  ...
  return{
    ...
    elapsedMs:0,
    startzeit:'',
    laststufen:def.laststufen.map(ls=>({label:ls.label,faktor:ls.f,druck:''})),
    messungen:def.intervals.map(min=>({min,druck:'',ablesung:'',versch:'',anm:''}))
  };
}

/* ────────── PERSISTENCE ────────── */
function saveDraftDebounced(){clearTimeout(_saveT);_saveT=setTimeout(()=>{try{localStorage.setItem(STORAGE_DRAFT,JSON.stringify(state));}catch(e){console.warn(e);}},500);}
function loadDraft(){
  try{
    const raw=localStorage.getItem(STORAGE_DRAFT);if(!raw)return;
...
function writeHistory(list){try{localStorage.setItem(STORAGE_HISTORY,JSON.stringify(list.slice(0,HISTORY_MAX)));}catch(e){console.warn(e);}}

function collectSnapshot(){return{savedAt:Date.now(),meta:clone(state.meta),vorgabe:clone(state.vorgabe),zyklen:clone(state.zyklen),settings:clone(state.settings)};}
...
function saveCurrentToHistory(){
  if(!state.meta.filiale){alert('Bitte Filiale wählen.');return false;}
...
function getAlarmAudioContext(){
  if(!_audioCtx){
    try{_audioCtx=new (window.AudioContext||window.webkitAudioContext)();_alarmGain=_audioCtx.createGain();_alarmGain.gain.value=1.0;_alarmGain.connect(_audioCtx.destination);}catch{return null;}
  }
  return _audioCtx;
}
function audioNeedsResume(ctx){return !ctx||ctx.state==='suspended'||ctx.state==='interrupted';}
async function unlockAlarmAudio(){
  const ctx=getAlarmAudioContext();if(!ctx)return false;
  try{
    if(audioNeedsResume(ctx)){await ctx.resume();await new Promise(r=>setTimeout(r,30));}
    if(audioNeedsResume(ctx)){_alarmReady=false;updateAlarmSoundButton();return false;}
    const buf=ctx.createBuffer(1,1,22050),src=ctx.createBufferSource();
    src.buffer=buf;src.connect(_alarmGain||ctx.destination);src.start(0);
    _alarmReady=true;updateAlarmSoundButton();return true;
  }catch{_alarmReady=false;updateAlarmSoundButton();return false;}
}
...
async function playIntervalBeep(){
  ...
  const ctx=getAlarmAudioContext();if(!ctx)return false;
  if(audioNeedsResume(ctx)){const ok=await unlockAlarmAudio();if(!ok)return false;}
  try{
...
function installAudioUnlock(){
  const fn=()=>{if(state.settings?.alarmSoundEnabled===false)return;void unlockAlarmAudio();};
  ['pointerdown','touchstart','keydown','click'].forEach(evt=>window.addEventListener(evt,fn,{passive:true}));
}
function updateAlarmSoundButton(){
  const btn=$('btnAlarmSoundToggle'),status=$('alarmSoundStatus');if(!btn)return;
  const enabledPref=state.settings.alarmSoundEnabled!==false,active=enabledPref&&_alarmReady;
  btn.textContent=active?'Ton ausschalten':'Ton einschalten';
  btn.classList.toggle('btn--save',active);btn.classList.toggle('btn--ghost',!active);
  if(status)status.textContent=active?'Alarmton ist aktiv.':(enabledPref?'Für iPhone: einmal hier oder vor dem Start direkt antippen.':'Alarmton ist ausgeschaltet.');
}
async function toggleAlarmSoundByUserGesture(){
  const active=state.settings.alarmSoundEnabled!==false&&_alarmReady;
  if(active){state.settings.alarmSoundEnabled=false;_alarmReady=false;updateAlarmSoundButton();saveDraftDebounced();return;}
  state.settings.alarmSoundEnabled=true;
  const ok=await unlockAlarmAudio();
  if(ok){const ctx=getAlarmAudioContext();const now=ctx.currentTime+0.02;scheduleBeep(ctx,now,0.08,2100,0.42);scheduleBeep(ctx,now+0.18,0.10,2550,0.50);}
  updateAlarmSoundButton();saveDraftDebounced();
  if(!ok)alert('Ton konnte nicht aktiviert werden. Lautstärke prüfen und erneut antippen.');
}

/* ────────── TIMER ────────── */
function getZyklusById(id){return state.zyklen.find(z=>z.id===id);}
function ensureTimer(zid,z){
  if(!timerMap[zid]){
    const min=Number(z?.elapsedMs||0)/60000;
    const ints=parseIntervalStr(z.intervalleStr);
    timerMap[zid]={running:false,startMs:0,accumulatedMs:Number(z?.elapsedMs||0),raf:null,alarmCount:ints.filter(iv=>iv>0&&min>=iv).length};
  }
  return timerMap[zid];
}
function getElapsedMs(zid,z){const t=timerMap[zid];if(!t)return Number(z?.elapsedMs||0);return t.running?t.accumulatedMs+(Date.now()-t.startMs):t.accumulatedMs;}
function updateTimerUi(card,z){
  if(!card||!z)return;
  const t=ensureTimer(z.id,z);const ms=getElapsedMs(z.id,z);z.elapsedMs=ms;
  const el=card.querySelector('[data-role="elapsed"]'),sb=card.querySelector('[data-role="timer-start"]'),tb=card.querySelector('[data-role="timer-stop"]');
  const sz=card.querySelector('[data-role="startzeit"]'),nx=card.querySelector('[data-role="naechstes"]');
  if(el)el.textContent=formatElapsed(ms);
  if(sz)sz.textContent=z.startzeit?`Startzeit: ${z.startzeit}`:'Noch nicht gestartet';
  if(sb){sb.textContent=t.running?'Läuft':(z.elapsedMs>0?'Weiter':'Start');sb.disabled=t.running;}
  if(tb)tb.disabled=!t.running;
  const ints=parseIntervalStr(z.intervalleStr),eMin=ms/60000;
  const next=ints.filter(iv=>iv>0).find(iv=>eMin<iv);
  if(nx)nx.textContent=next!==undefined?`Nächste Messung: ${next} min (in ${Math.max(0,Math.ceil((next*60000-ms)/1000))}s)`:'Alle Intervalle erreicht';
  ...
  if(idx>=0){const r=card.querySelector(`tr[data-row="${idx}"]`);if(r)r.classList.add('row-active');}
}
function triggerIntervalAlarm(zid){
  ...
  if(card){card.classList.remove('zyklus-card--alarm');void card.offsetWidth;card.classList.add('zyklus-card--alarm');}
  ...
  setTimeout(()=>{if(card)card.classList.remove('zyklus-card--alarm');if(display)display.classList.remove('timer-display--alarm');},Math.max(2400,Number(state.settings.alarmDurationSec||4)*1000+600));
}
function tickTimer(zid){
  const z=getZyklusById(zid),t=timerMap[zid];if(!z||!t||!t.running)return;
  const card=document.querySelector(`.zyklus-card[data-zid="${zid}"]`);
  z.elapsedMs=getElapsedMs(zid,z);if(card)updateTimerUi(card,z);
  const ints=parseIntervalStr(z.intervalleStr).filter(n=>n>0);
  const passed=ints.filter(iv=>z.elapsedMs/60000>=iv).length;
  if(passed>t.alarmCount){t.alarmCount=passed;triggerIntervalAlarm(zid);}
  updateFloatingTimerWidget();
  t.raf=requestAnimationFrame(()=>tickTimer(zid));
}
function startTimer(zid){
  const z=getZyklusById(zid);if(!z)return;
  if(state.settings.alarmSoundEnabled!==false)void unlockAlarmAudio();
  const t=ensureTimer(zid,z);if(t.running)return;
  ...
  const card=document.querySelector(`.zyklus-card[data-zid="${zid}"]`);
  updateTimerUi(card,z);tickTimer(zid);startFloatingLoop();saveDraftDebounced();
}
function stopTimer(zid){
  const z=getZyklusById(zid),t=timerMap[zid];if(!z||!t||!t.running)return;
  t.accumulatedMs+=(Date.now()-t.startMs);z.elapsedMs=t.accumulatedMs;t.running=false;
  if(t.raf)cancelAnimationFrame(t.raf);t.raf=null;
  const card=document.querySelector(`.zyklus-card[data-zid="${zid}"]`);
  updateTimerUi(card,z);updateFloatingTimerWidget();stopFloatingLoopIfIdle();saveDraftDebounced();
}
function resetTimer(zid){
  const z=getZyklusById(zid);if(!z)return;
  const t=ensureTimer(zid,z);if(t.raf)cancelAnimationFrame(t.raf);
  t.running=false;t.startMs=0;t.accumulatedMs=0;t.raf=null;t.alarmCount=0;
  z.elapsedMs=0;z.startzeit='';
  const card=document.querySelector(`.zyklus-card[data-zid="${zid}"]`);
  updateTimerUi(card,z);updateFloatingTimerWidget();stopFloatingLoopIfIdle();saveDraftDebounced();
}

/* ────────── FLOATING TIMER ────────── */
function getFirstRunningZyklus(){return state.zyklen.find(z=>timerMap[z.id]?.running)||null;}
function isElementVisible(el){if(!el)return false;const r=el.getBoundingClientRect();if(r.width===0&&r.height===0)return false;if(getComputedStyle(el).display==='none')return false;return r.top>=0&&r.bottom<=window.innerHeight;}
function updateFloatingTimerWidget(){
  const wrap=$('floatingTimer'),label=$('floatingTimerLabel'),display=$('floatingTimerDisplay');if(!wrap||!label||!display)return;
  const z=getFirstRunningZyklus();if(!z){wrap.hidden=true;return;}
  const card=document.querySelector(`.zyklus-card[data-zid="${z.id}"]`);
  const timerBox=card?.querySelector('.timer-box');
  label.textContent=`Zyklus ${z.nr}`;display.textContent=formatElapsed(getElapsedMs(z.id,z));
...
/* ────────── TIME ADJUST MODAL ────────── */
function openTimeAdjustModal(zid){_timeAdjustVid=zid;$('timeAdjustInput').value='0';updateTimeAdjustPreview();$('timeAdjustModal').hidden=false;}
function closeTimeAdjustModal(){$('timeAdjustModal').hidden=true;_timeAdjustVid=null;}
function updateTimeAdjustPreview(){
  const z=getZyklusById(_timeAdjustVid);if(!z)return;
...
function applyTimeAdjustment(){
  ...
  z.elapsedMs=next;if(!z.startzeit&&next>0)z.startzeit=formatTimeHHMMSS(new Date());
  const card=document.querySelector(`.zyklus-card[data-zid="${z.id}"]`);
...
function buildLaststufenHtml(z){
  ...
  return z.laststufen.map((ls,i)=>{
    const lastKn=Number.isFinite(Pp)?(Pp*ls.faktor):NaN;
...
function buildZyklusHtml(z){
  return `<div class="zyklus-card" data-zid="${h(z.id)}">
...
function renderZyklen(){
  const host=$('zyklenContainer');if(!host)return;
  if(!state.zyklen.length){host.innerHTML=`<div class="empty-state">Noch kein Lastzyklus angelegt. Über den Plus-Button hinzufügen.</div>`;updateFloatingTimerWidget();return;}
...
function hookZyklenDelegation(){
  const host=$('zyklenContainer');if(!host||host.dataset.bound==='1')return;host.dataset.bound='1';
  host.addEventListener('input',e=>{
    const el=e.target.closest('[data-role]');if(!el)return;
    const card=el.closest('.zyklus-card');if(!card)return;
    const z=getZyklusById(card.dataset.zid);if(!z)return;
...
if(role==='m-versch'){
  ...
  host.addEventListener('click',e=>{
    ...
    if(role==='timer-reset'){if(confirm('Timer zurücksetzen?'))resetTimer(z.id);return;}
    if(role==='elapsed')return openTimeAdjustModal(z.id);
    if(role==='zyklus-del'){if(confirm(`Zyklus ${z.nr} löschen?`)){state.zyklen=state.zyklen.filter(x=>x.id!==z.id);delete timerMap[z.id];renderZyklen();saveDraftDebounced();}return;}
    if(role==='row-plus'){
      ...
      z.messungen.push({min:last+step,druck:'',ablesung:'',versch:'',anm:''});
      z.intervalleStr=z.messungen.map(m=>m.min).join(', ');
      renderZyklen();saveDraftDebounced();return;
    }
    if(role==='m-anm-btn'){
      const cur=z.messungen[idx]?.anm||'';
      const v=prompt('Anmerkung:',cur);
      if(v!==null){z.messungen[idx].anm=v;renderZyklen();saveDraftDebounced();}
      return;
    }
  });
}
...
function reconcileMessungen(z){
  ...
  z.messungen=ints.map(min=>map[min]||{min,druck:'',ablesung:'',versch:'',anm:''});
}

/* ────────── BERECHNUNGEN ────────── */
function calcLapp(deltaS_mm,Pp_kN,Pa_kN,Et_kNmm2,At_mm2){
  // Lapp [m] = (ΔL[mm] * Et[kN/mm²] * At[mm²]) / ((Pp - Pa)[kN] * 1000)
...
function calcKriechmass(points){
  // points = [{min, s_mm}], gibt ks = (s2-s1)/log10(t2/t1) zurück
  if(!Array.isArray(points)||points.length<2)return NaN;
  const sorted=points.filter(p=>Number.isFinite(p.min)&&p.min>0&&Number.isFinite(p.s_mm)).sort((a,b)=>a.min-b.min);
...
function getZyklusKpis(z){
  const Pp=Number(state.vorgabe.Pp),Pa=Number(state.vorgabe.Pa);
  const Et=Number(state.vorgabe.Et),At=Number(state.vorgabe.At);
  const halt=z.haltMin;
  // Verschiebung bei Halt-Laststufe relativ zu Pa: nimm Eingabe versch direkt (Excel-Logik)
  const versch0=Number(z.messungen[0]?.versch);
  const verschHalt=Number(z.messungen[z.messungen.length-1]?.versch); // letzter Punkt während Halt
  let dS=NaN;
  // Bessere Logik: vergleiche Wert bei min=0 und min=halt im Halteintervall
  const m0=z.messungen.find(m=>Number(m.min)===0),mH=z.messungen.find(m=>Number(m.min)===halt);
  ...
  // Lapp nur sinnvoll für Pp-Halt
  const Lapp=(haltLs?.faktor===1.0)?calcLapp(dS,Pp,Pa,Et,At):NaN;
  // Kriechmaß: Punkte im Bereich 20–60 (nicht-bindig) oder 60–180 (bindig)
  const isB=state.vorgabe.bodenart==='bindig';
  const tMin=isB?60:20,tMax=isB?180:60;
  const krPts=z.messungen.filter(m=>Number(m.min)>=tMin&&Number(m.min)<=tMax).map(m=>({min:Number(m.min),s_mm:Number(m.versch)}));
  const ks=calcKriechmass(krPts);
  return{lastKn,dS,Lapp,ks,haltLabel:haltLs?.label||'—'};
}

/* ────────── DIAGRAMM (SVG) ────────── */
function buildLastVerschiebungSvg(z){
  // X: Last [kN], Y: Verschiebung [mm], inkl. Grenzlinien min/max Lapp
  const Pp=Number(state.vorgabe.Pp),Pa=Number(state.vorgabe.Pa);
  const Et=Number(state.vorgabe.Et),At=Number(state.vorgabe.At);
  const Ltf=Number(state.vorgabe.Ltf),Le=Number(state.vorgabe.Le),Ltb=Number(state.vorgabe.Ltb);
  const minLapp=0.8*Ltf+Le,maxLapp=Ltf+Le+0.5*Ltb;
  // Punkte: für jede Laststufe der höchste Min-Wert (also nach Verschiebung in Halt)
  const pts=z.laststufen.map((ls,i)=>{
    const last=Pp*ls.faktor;
    // Wir nehmen Verschiebung beim Min=0 der jeweiligen Stufe – vereinfacht: erster Messwert bei dieser Stufe = Stufenwechsel
    // In dieser App speichern wir nur die Halt-Stufe in der Tabelle; daher hier nur Halt-Punkt
    return{last,s:NaN};
  });
  // Halt-Punkt einfügen
  ...
  // Grenzlinien: s = Lapp * (F-Pa) / (Et*At) * 1000  → in mm
  function sFor(F,Lapp){if(!Number.isFinite(F)||!Number.isFinite(Lapp)||!Number.isFinite(Et)||!Number.isFinite(At))return NaN;return (Lapp*(F-Pa)*1000)/(Et*At);}
  const linePts=[];
  for(let f=Pa;f<=xMax;f+=Math.max(0.1,xMax/40))linePts.push(f);
  const polyMin=linePts.map(f=>{const s=sFor(f,minLapp);return Number.isFinite(s)?`${tx(f)},${ty(Math.max(0,Math.min(yMax,s)))}`:null;}).filter(Boolean).join(' ');
  const polyMax=linePts.map(f=>{const s=sFor(f,maxLapp);return Number.isFinite(s)?`${tx(f)},${ty(Math.max(0,Math.min(yMax,s)))}`:null;}).filter(Boolean).join(' ');
  // Achsen-Ticks
...
function buildKriechSvg(z){
  // X: log(t), Y: s [mm], während Halt
  const W=520,H=260,ml=58,mr=20,mt=20,mb=42,pw=W-ml-mr,ph=H-mt-mb;
  const pts=z.messungen.filter(m=>Number(m.min)>0&&Number.isFinite(Number(m.versch))).map(m=>({t:Number(m.min),s:Number(m.versch)})).sort((a,b)=>a.t-b.t);
  const tMax=Math.max(...pts.map(p=>p.t),z.haltMin||60);
  const sMin=Math.min(...pts.map(p=>p.s),0),sMax=Math.max(...pts.map(p=>p.s),sMin+1);
  const tMin=1;
...
function renderAuswertung(){
  const host=$('auswertungContainer');if(!host)return;
  collectVorgabeFromUi();
  if(!state.zyklen.length){host.innerHTML=`<div class="empty-state">Noch keine Lastzyklen vorhanden.</div>`;return;}
  const isB=state.vorgabe.bodenart==='bindig';
  const ksLimit=2;
  host.innerHTML=state.zyklen.map(z=>{
    const k=getZyklusKpis(z);
    const Ltf=Number(state.vorgabe.Ltf),Le=Number(state.vorgabe.Le),Ltb=Number(state.vorgabe.Ltb);
    const minL=0.8*Ltf+Le,maxL=Ltf+Le+0.5*Ltb;
    const lappOk=Number.isFinite(k.Lapp)?(k.Lapp>=minL&&k.Lapp<=maxL):null;
    const ksOk=Number.isFinite(k.ks)?k.ks<=ksLimit:null;
    return `<div class="auswertung-block">
...
function renderHistoryList(){
  ...
  const list=readHistory();
  if(!list.length){host.innerHTML=`<div class="empty-state">Noch keine Protokolle gespeichert.</div>`;return;}
  host.innerHTML=list.map(e=>{
    const s=e.snapshot||{};
    return `<details class="historyItem" data-hid="${h(e.id)}">
...
function hookHistoryDelegation(){
  const host=$('historyList');if(!host||host.dataset.bound==='1')return;host.dataset.bound='1';
  host.addEventListener('click',async e=>{
    const btn=e.target.closest('[data-hact]');if(!btn)return;
    const id=btn.dataset.id,act=btn.dataset.hact;
    const list=readHistory(),entry=list.find(x=>x.id===id);if(!entry&&act!=='del')return;
    if(act==='del'){if(!confirm('Eintrag löschen?'))return;writeHistory(list.filter(x=>x.id!==id));renderHistoryList();return;}
    if(act==='load'){applySnapshot(entry.snapshot,true);saveDraftDebounced();document.querySelector('.tab[data-tab="formular"]')?.click();return;}
    if(act==='pdf'){await exportPdfFromSnapshot(entry.snapshot);return;}
    if(act==='export'){downloadJson(entry.snapshot,`${dateTag()}_HTB_Anker_${(entry.snapshot.meta?.ankerNr||'export').replace(/\s+/g,'_')}.json`);return;}
  });
}
...
function buildTemplateSnapshot(){
  const snap=collectSnapshot();
  // Vorlage: Werte in Messreihen löschen, Stammdaten leer
  snap.meta={...snap.meta,bauvorhaben:'',bauherr:'',ankerlage:'',ankerNr:'',blattNr:'',pruefdatum:'',anmerkung:''};
  snap.zyklen=(snap.zyklen||[]).map(z=>({...z,elapsedMs:0,startzeit:'',laststufen:z.laststufen.map(ls=>({...ls,druck:''})),messungen:z.messungen.map(m=>({...m,druck:'',ablesung:'',versch:'',anm:''}))}));
  return snap;
}

/* ────────── PDF EXPORT (mit Filiale-Fußzeile) ────────── */
async function exportPdfFromSnapshot(snap){
  ...
  page.drawText('HTB Ankerprüfung – Verpreßanker Eignungsprüfung',{x:30,y:H-20,size:11,font:fontB,color:rgb(1,1,1)});
  // Stammdaten
  y=H-60;
  const meta=snap.meta||{},vor=snap.vorgabe||{};
  const lines=[
    ['Bauvorhaben',meta.bauvorhaben||''],
    ['Bauherr',meta.bauherr||''],
    ['Anker Nr.',meta.ankerNr||''],['Ankerlage',meta.ankerlage||''],
    ['Prüfdatum',dateDE(meta.pruefdatum)],['Bodenart',vor.bodenart||''],
    ['Ankertyp',vor.ankertyp||''],['LA / Ltb / Ltf / Le [m]',`${vor.LA||''} / ${vor.Ltb||''} / ${vor.Ltf||''} / ${vor.Le||''}`],
    ['Pp / Pa [kN]',`${vor.Pp||''} / ${vor.Pa||''}`],['Et / At',`${vor.Et||''} kN/mm² / ${vor.At||''} mm²`]
  ];
  page.drawText('Stammdaten & Vorgabe',{x:30,y,size:12,font:fontB,color:rgb(0,0,0)});y-=16;
  lines.forEach(([k,v])=>{page.drawText(`${k}:`,{x:30,y,size:9,font:fontB});page.drawText(String(v),{x:160,y,size:9,font});y-=12;});
  // Zyklen-Tabelle
  y-=10;page.drawText('Lastzyklen',{x:30,y,size:12,font:fontB});y-=14;
  (snap.zyklen||[]).forEach(z=>{
    if(y<140){const np=pdf.addPage([595.28,841.89]);page=np;y=H-50;}
    page.drawText(`Zyklus ${z.nr} · Halt ${z.haltMin} min`,{x:30,y,size:10,font:fontB});y-=12;
    page.drawText('Min   Druck   Mess.   Verschieb.   Anm.',{x:30,y,size:8,font:fontB});y-=10;
    z.messungen.forEach(m=>{
      if(y<80){const np=pdf.addPage([595.28,841.89]);page=np;y=H-50;}
      page.drawText(`${String(m.min).padEnd(5)} ${String(m.druck||'').padEnd(7)} ${String(m.ablesung||'').padEnd(7)} ${String(m.versch||'').padEnd(11)} ${m.anm||''}`,{x:30,y,size:8,font});y-=10;
    });
    y-=6;
  });
  // Footer mit Filiale
  ...
  pages.forEach(p=>{
    ...
    p.drawText(`HTB Baugesellschaft m.b.H. – Filiale ${meta.filiale||'—'}`,{x:30,y:18,size:8,font:fontB,color:rgb(1,1,1)});
    p.drawText(`${fil.adresse||''} · ${fil.tel||''} · ${fil.email||''}`,{x:30,y:8,size:7,font,color:rgb(.85,.9,1)});
  });
  const bytes=await pdf.save();
  const blob=new Blob([bytes],{type:'application/pdf'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download=`${dateTag()}_HTB_Ankerpruefung_${(meta.ankerNr||'export').replace(/\s+/g,'_')}.pdf`;a.click();
  setTimeout(()=>URL.revokeObjectURL(url),30000);
}
...
function init(){
  loadDraft();
  syncMetaToUi();syncVorgabeToUi();renderZyklen();updateLappPreview();renderHistoryList();
  // Tabs
  ...
  $('btnAddZyklus').addEventListener('click',()=>{
    ...
    renderZyklen();saveDraftDebounced();
  });
  ...
  $('btnSaveSettings').addEventListener('click',()=>{state.settings.alarmDurationSec=clamp(Number($('settings-alarmDuration').value||4),1,30);saveDraftDebounced();alert('Einstellungen gespeichert.');});
  $('btnReset').addEventListener('click',()=>{
    if(!confirm('Komplettes Formular zurücksetzen?'))return;
    Object.assign(state,getInitialState());
    Object.keys(timerMap).forEach(k=>{const t=timerMap[k];if(t.raf)cancelAnimationFrame(t.raf);delete timerMap[k];});
    syncMetaToUi();syncVorgabeToUi();renderZyklen();updateLappPreview();saveDraftDebounced();
  });
  ...
  $('settings-alarmDuration').addEventListener('input',e=>{state.settings.alarmDurationSec=clamp(Number(e.target.value||4),1,30);saveDraftDebounced();});
  $('btnExportTemplate').addEventListener('click',()=>{const s=buildTemplateSnapshot();downloadJson(s,`${dateTag()}_HTB_Anker_Vorlage.htbanker.json`);});
  $('btnImportTemplate').addEventListener('click',()=>$('importFileInput').click());
  $('importFileInput').addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f)return;try{const t=await f.text();applySnapshot(JSON.parse(t),true);saveDraftDebounced();alert('Vorlage importiert.');}catch{alert('Import fehlgeschlagen.');}finally{e.target.value='';}});
  $('btnExportFull').addEventListener('click',()=>downloadJson(collectSnapshot(),`${dateTag()}_HTB_Anker_Export.json`));
  ...
  document.querySelectorAll('.modal-adj-btn').forEach(b=>b.addEventListener('click',()=>{$('timeAdjustInput').value=String(Number($('timeAdjustInput').value||0)+Number(b.dataset.adj||0));updateTimeAdjustPreview();}));
