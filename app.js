...
function showUpdateBanner(){
  ...
  banner.innerHTML = `
    ...
    <button onclick="navigator.serviceWorker.getRegistration().then(r=>r?.waiting?.postMessage({action:'skipWaiting'}));this.closest('#updateBanner').remove();"
      style="background:#f08a1c;color:#fff;border:0;border-radius:6px;padding:6px 12px;cursor:pointer;font-weight:700;font-size:13px">Jetzt laden</button>
    <button onclick="this.closest('#updateBanner').remove();"
      style="background:transparent;border:1px solid rgba(255,255,255,.3);color:#fff;border-radius:6px;padding:6px 10px;cursor:pointer;font-size:12px">Später</button>
  `;
...
/* ═══════════════════════════════════════════════════════
   DATUM-NORMALISIERUNG  (Fix für "Invalid Date")
═══════════════════════════════════════════════════════ */
function normalizeCsvDate(value){
  const s = String(value||'').trim();
  if(!s) return '';

  // Bereits ISO: 2026-01-26
  if(/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;

  // Deutsch: 26.01.2026 oder 26/01/2026
  let m = s.match(/^(\d{1,2})[./](\d{1,2})[./](\d{4})$/);
  if(m){
    return `${m[3]}-${String(m[2]).padStart(2,'0')}-${String(m[1]).padStart(2,'0')}`;
  }

  // Deutsch kurz: 26.01.26
...
function loadAllKalibs(){
  ...
  return [...BUILTIN_KALIBRIERUNGEN, ...local.filter(k=>!builtinIds.has(k.id))];
}

function saveUserKalibs(kalibs){
  const builtinIds = new Set(BUILTIN_KALIBRIERUNGEN.map(k=>k.id));
  try{ localStorage.setItem(STORAGE_KALIB, JSON.stringify(kalibs.filter(k=>!builtinIds.has(k.id)))); }
...
function kalibStatus(k){
  ...
  if(!bis) return 'ok';
  const diff = Math.floor((bis - new Date())/86400000);
...
function parseKalibCsv(text){
  ...
  for(const line of lines){
    ...
    if(!inPoints){
      ...
      if(parts.length>=2){
        ...
        if(Number.isFinite(kN)&&Number.isFinite(bar)) punkte.push({kN,bar});
      }
    }
  }
  ...
  if(!displayName||!presseNr||!kalibriertAm||punkte.length<2){
    throw new Error(
      'CSV unvollständig.\nPflichtfelder: displayName, presseNr, kalibriertAm (Format YYYY-MM-DD), mindestens 2 Punkte.'
    );
  }

  return {
    id: `${presseNr}_${kalibriertAm}`,
    displayName, presseTyp, presseNr,
    manometerTyp, manometerNr, kalibriertAm,
...
function interpoliereBar(zielKn, punkte){
  ...
  if(!Number.isFinite(zielKn)||zielKn<minKn||zielKn>maxKn) return {bar:null,oor:true};
  ...
  for(let i=0;i<sorted.length-1;i++){
    if(sorted[i].kN<=zielKn&&sorted[i+1].kN>=zielKn){ lo=sorted[i]; hi=sorted[i+1]; break; }
  }

  const t = (zielKn-lo.kN)/(hi.kN-lo.kN);
  return {bar: Math.round((lo.bar+t*(hi.bar-lo.bar))*10)/10, oor:false};
}
...
function berechneDruckvorschau(){
  ...
  return stufen.map(s=>{
    if(!kalib||!Number.isFinite(s.kN)||s.kN<0) return {...s,bar:null,oor:false,noKalib:!kalib};
...
function getInitialState(){
  return {
    meta:{
      ...
      pumpeNr:'', anmerkung:'',
      selectedKalibId:'', activeZyklusId:''
    },
    vorgabe:{
      bodenart:'nichtbindig', ankertyp:'',
      LA:'', Ltb:'', Ltf:'', Le:'',
      Et:'195', At:'', P0:'', Pa:'0', Pp:'',
      Pt01k:'', Pd:'', gamma:'1.1'
    },
    zyklen:[],
...
const timerMap = {};
let _saveT=null, _audioCtx=null, _alarmGain=null, _alarmReady=false,
    _floatingRaf=null, _timeAdjustVid=null;
...
function ensureDefaultZyklen(){
  ...
  if(state.zyklen.length===0){
    for(let i=1;i<=5;i++) state.zyklen.push(defaultZyklus(i));
  }
  ensureActiveZyklus();
}

function ensureActiveZyklus(){
  if(!state.zyklen.length){ state.meta.activeZyklusId=''; return null; }
  let z = state.zyklen.find(x=>x.id===state.meta.activeZyklusId)||null;
  if(!z){ z=state.zyklen[0]; state.meta.activeZyklusId=z.id; }
  return z;
}

function getActiveZyklus(){ return ensureActiveZyklus(); }
...
═══════════════════════════════════════════════════════ */
const META_FIELDS = [
  ['meta-filiale','filiale'],['meta-bauvorhaben','bauvorhaben'],
  ['meta-bauherr','bauherr'],['meta-bauleitung','bauleitung'],
  ['meta-ankerlage','ankerlage'],['meta-ankerNr','ankerNr'],
  ['meta-blattNr','blattNr'],['meta-pruefdatum','pruefdatum'],
  ['meta-pumpeNr','pumpeNr'],['meta-anmerkung','anmerkung']
];

const VOR_FIELDS = ['ankertyp','LA','Ltb','Ltf','Le','Et','At','P0','Pa','Pp','Pt01k','Pd','gamma'];

function syncMetaToUi(){
  META_FIELDS.forEach(([id,k])=>{ const el=$(id); if(el) el.value=state.meta[k]||''; });
  const sel=$('presseSelect');
  if(sel) sel.value=state.meta.selectedKalibId||'';
}

function collectMetaFromUi(){
  META_FIELDS.forEach(([id,k])=>{ const el=$(id); if(el) state.meta[k]=el.value||''; });
  const sel=$('presseSelect');
...
function updateComputedRowsForCard(card,z){
  if(!card||!z) return;
  z.messungen.forEach((m,idx)=>{
    const inp = card.querySelector(`[data-role="m-versch"][data-row="${idx}"]`);
    if(inp){
      inp.value   = m.versch||'';
      inp.readOnly = true;
      inp.classList.add('mess-input--auto');
      inp.title = idx===0
...
/* ═══════════════════════════════════════════════════════
   ...
   FIX: Pp steht nicht mehr doppelt — kein separates Anzeigefeld
═══════════════════════════════════════════════════════ */
function getMeasuredLappForDisplay(){
  const zs=[...state.zyklen].sort((a,b)=>b.nr-a.nr);
  for(const z of zs){
    const k=getZyklusKpis(z);
    if(Number.isFinite(k.Lapp)) return {Lapp:k.Lapp,zyklus:z.nr,dS:k.dS};
  }
...
function computeVorgabeAuto(){
  ...
  const sGrenzB = (Number.isFinite(minLapp)&&Number.isFinite(deltaF)&&deltaF>0&&Number.isFinite(Et)&&Et>0&&Number.isFinite(At)&&At>0)
    ? (minLapp*deltaF*1000)/(Et*At) : NaN;
  const sGrenzA = (Number.isFinite(maxLapp)&&Number.isFinite(deltaF)&&deltaF>0&&Number.isFinite(Et)&&Et>0&&Number.isFinite(At)&&At>0)
  ...
  return {minLapp,maxLapp,sGrenzB,sGrenzA,PpFromPd,PdFromPp,
          measuredLapp:measured.Lapp,measuredZyklus:measured.zyklus};
}

function maybeAutofillVorgabe(changedKey){
  const a = computeVorgabeAuto();

  // Pp aus Pd·γa — nur wenn Pp noch leer
  if((changedKey==='Pd'||changedKey==='gamma') &&
      (!state.vorgabe.Pp||String(state.vorgabe.Pp).trim()==='') &&
      Number.isFinite(a.PpFromPd)){
    state.vorgabe.Pp = formatInputNumber(a.PpFromPd,1);
    const el=$('vor-Pp');
    if(el) el.value=state.vorgabe.Pp;
  }

  // Pd aus Pp/γa — nur wenn Pd noch leer
  if((changedKey==='Pp'||changedKey==='gamma') &&
      (!state.vorgabe.Pd||String(state.vorgabe.Pd).trim()==='') &&
      Number.isFinite(a.PdFromPp)){
    state.vorgabe.Pd = formatInputNumber(a.PdFromPp,1);
    const el=$('vor-Pd');
    if(el) el.value=state.vorgabe.Pd;
  }
}
...
function updateLappPreview(){
  ...
  // Pp-Hint: zeigt an was auto-berechnet wurde — kein separates Anzeigefeld mehr
...
async function unlockAlarmAudio(){
  ...
  try{
    ...
    src.connect(_alarmGain||ctx.destination); src.start(0);
...
function updateAlarmSoundButton(){
  const btn=$('btnAlarmSoundToggle'), status=$('alarmSoundStatus');
  if(!btn) return;
  const enabled=state.settings.alarmSoundEnabled!==false;
  const active=enabled&&_alarmReady;
  btn.textContent=active?'Ton ausschalten':'Ton einschalten';
  btn.classList.toggle('btn--save',active);
  btn.classList.toggle('btn--ghost',!active);
  if(status) status.textContent=active?'Alarmton ist aktiv.':(enabled?'Für iPhone: einmal hier antippen.':'Alarmton ist ausgeschaltet.');
}
...
async function toggleAlarmSoundByUserGesture(){
  ...
  if(ok){ const ctx=getAlarmAudioContext(),now=ctx.currentTime+0.02; scheduleBeep(ctx,now,0.08,2100,0.42); scheduleBeep(ctx,now+0.18,0.10,2550,0.50); }
...
function ensureTimer(zid,z){
  if(!timerMap[zid]){
    ...
    timerMap[zid]={running:false,startMs:0,accumulatedMs:Number(z?.elapsedMs||0),raf:null,
...
function highlightActiveMeasurementRow(z){
  ...
  if(!z) return;
  const card=document.querySelector(`.zyklus-card[data-zid="${z.id}"]`); if(!card) return;
  const ints=parseIntervalStr(z.intervalleStr);
...
function updateGlobalTimerUi(){
  ...
  const display=$('globalTimerDisplay'), badge=$('activeZyklusBadge'),
        sel=$('activeZyklusSelect'), startzeit=$('globalTimerStartzeit'),
        nextEl=$('globalTimerNext'), btnStart=$('btnGlobalTimerStart'),
        btnStop=$('btnGlobalTimerStop');

  if(!z){ if(display) display.textContent='00:00'; return; }
  ...
  if(nextEl) nextEl.textContent=next!==undefined
    ?`Nächste Messung: ${next} min (in ${Math.max(0,Math.ceil((next*60000-ms)/1000))}s)`
    :'Alle Intervalle erreicht';

  if(btnStart){ btnStart.textContent=t.running?'Läuft':(z.elapsedMs>0?'Weiter':'Start'); btnStart.disabled=t.running; }
...
function triggerIntervalAlarm(){
  ...
  setTimeout(()=>{ if(display) display.classList.remove('timer-display--alarm'); },
    Math.max(2400,Number(state.settings.alarmDurationSec||4)*1000+600));
}

function tickTimer(zid){
  const z=getZyklusById(zid), t=timerMap[zid];
  ...
  if(passed>t.alarmCount){ t.alarmCount=passed; triggerIntervalAlarm(); }
  updateGlobalTimerUi();
...
function stopAllOtherTimers(exceptZid){
  Object.keys(timerMap).forEach(zid=>{ if(zid!==exceptZid&&timerMap[zid]?.running) stopTimer(zid); });
}
...
function startTimer(zid){
  ...
  updateGlobalTimerUi(); tickTimer(zid); startFloatingLoop(); saveDraftDebounced();
}
...
function stopTimer(zid){
  ...
  updateGlobalTimerUi(); updateFloatingTimerWidget(); stopFloatingLoopIfIdle(); saveDraftDebounced();
}
...
function startActiveTimer(){ const z=getActiveZyklus(); if(z) startTimer(z.id); }
function stopActiveTimer(){  const z=getActiveZyklus(); if(z) stopTimer(z.id); }
function resetActiveTimer(){ const z=getActiveZyklus(); if(z&&confirm(`Timer für Zyklus ${z.nr} zurücksetzen?`)) resetTimer(z.id); }
...
function stopFloatingLoopIfIdle(){
  if(!Object.values(timerMap).some(t=>t.running)&&_floatingRaf){
    cancelAnimationFrame(_floatingRaf); _floatingRaf=null;
  }
}
...
function applyTimeAdjustment(){
  ...
  if(t.running){ t.startMs=Date.now(); t.accumulatedMs=next; }
  else t.accumulatedMs=next;
  z.elapsedMs=next;
  if(!z.startzeit&&next>0) z.startzeit=formatTimeHHMMSS(new Date());
  updateGlobalTimerUi(); updateFloatingTimerWidget(); saveDraftDebounced(); closeTimeAdjustModal();
}
...
function renderPresseDropdown(){
  const sel=$('presseSelect'); if(!sel) return;
  const current=state.meta.selectedKalibId;
  sel.innerHTML=`<option value="">— keine ausgewählt —</option>`;
  loadAllKalibs().forEach(k=>{
    const icon=kalibStatus(k)==='ok'?'✅':kalibStatus(k)==='warn'?'⚠️':'❌';
...
function renderKalibInfo(){
  const box=$('kalibInfoBox'), emptyHint=$('kalibEmptyHint'), preview=$('kalibPreview');
  if(!box) return;
  const kalib=findKalibById(state.meta.selectedKalibId);
  ...
  if(badge){
    badge.textContent=status==='ok'?`✅ Gültig bis ${bisStr}`:
...
function renderKalibPreview(){
  const wrap=$('kalibPreview'), table=$('kalibPreviewTable');
  if(!wrap||!table) return;
  const Pp=Number(state.vorgabe.Pp), kalib=findKalibById(state.meta.selectedKalibId);
  if(!kalib||!Number.isFinite(Pp)||Pp<=0){ wrap.hidden=true; return; }
  wrap.hidden=false;
  ...
  table.innerHTML=`
    ...
    ${vorschau.map(v=>{
      ...
      return `<div class="kalib-prev-row">
...
function getZyklusKpis(z){
  ...
  if(m0 && mH && Number.isFinite(Number(m0.versch)) && Number.isFinite(Number(mH.versch))){
    // da Versch jetzt Schrittwerte sind, aufsummieren von 0..halt
  ...
  // kumulative Verschiebung für Kriechmaß
  let cum = 0;
  const cumPts = z.messungen.map(m=>{
    const v = Number(m.versch);
    if(Number.isFinite(v)) cum += v;
    return { min:Number(m.min), s_mm:Number.isFinite(cum) ? cum : NaN };
  });
...
function buildLastVerschiebungSvg(z){
  ...
  const linePts=[]; for(let f=Pa;f<=xMax;f+=Math.max(0.1,xMax/40)) linePts.push(f);
...
async function exportPdfFromSnapshot(snap){
  ...
  y-=10;
  page.drawText('Lastzyklen',{x:30,y,size:12,font:fontB}); y-=14;

  (snap.zyklen||[]).forEach(z=>{
    if(y<140){ page=pdf.addPage([595.28,841.89]); y=H-50; }
    ...
    z.messungen.forEach(m=>{
      if(y<80){ page=pdf.addPage([595.28,841.89]); y=H-50; }
      page.drawText(
...
function init(){
  ...
  if(state.meta.selectedKalibId) syncDruckFromKalib();
  ...
  $('btnAddZyklus')?.addEventListener('click',()=>{
    if(state.zyklen.length>=5){ alert('Es sind bereits 5 Lastzyklen vorhanden.'); return; }
  ...
  document.querySelectorAll('.modal-adj-btn').forEach(b=>b.addEventListener('click',()=>{
    const input=$('timeAdjustInput'); if(!input) return;
  ...
  if('serviceWorker' in navigator){
    navigator.serviceWorker.register(`${BASE_PATH}sw.js?v=1`)
      .then(reg=>{
